/* Audrey Closet v13.24 — Smart Scan Phase 6.3A6 Hybrid Pattern Comparator
 * Compares Phase 5.3 baseline, current Calibration Lab, broad Phase 5.4 spatial model,
 * focused four-patch Solid/Graphic, and the hybrid structure-first experiment.
 */
(function(){
  'use strict';
  const VERSION='13.24-phase6.3a6-hybrid-compare1';
  ...
  function renderCombined(report,spatial,solidGraphic,hybrid){
    ...
    host.innerHTML=`<div class="ssc-result-grid"><div class="ssc-result-card"><h5>Phase 5.3 baseline</h5><p>Pattern: ${esc(report?.baseline?.pattern||'—')}</p><p>Color: ${esc(report?.baseline?.color||'—')}</p><p>Category: ${esc(report?.baseline?.category||'—')}</p></div><div class="ssc-result-card"><h5>Current Calibration</h5><p>Pattern: ${esc(report?.calibrated?.pattern||'—')}</p><p>Color: ${esc(report?.calibrated?.color||'—')}</p><p>Category: ${esc(report?.calibrated?.category||'—')}</p></div></div><div class="ssc-result-grid" style="margin-top:8px"><div class="ssc-result-card"><h5>Phase 5.4 Spatial</h5><p>Pattern: ${esc(spatial?.pattern||'—')}</p><p>Confidence: ${pct(spatial?.confidence||0)}</p></div><div class="ssc-result-card"><h5>Phase 5.4B Four-Patch</h5><p>Pattern: ${esc(solidGraphic?.pattern||'—')}</p><p>Confidence: ${pct(solidGraphic?.confidence||0)}</p></div></div><div class="ssc-result-card" style="margin-top:8px"><h5>Phase 5.4C Hybrid — candidate local flow</h5><p class="ssc-change">Pattern: ${esc(hybrid?.pattern||'—')}</p><p>Confidence: ${pct(hybrid?.confidence||0)}</p><p>Stage: ${esc(hybrid?.stage||'—')}</p><p>Reason: ${esc(hybrid?.reason||'—')}</p></div><div class="ssc-metrics"><strong>Four-patch Solid/Graphic evidence</strong><br>Edge distance: ${num(edges.distance)} · edges match: ${edges.match?'yes':'no'} · edge stable: ${sg.edgeStable?'yes':'no'}<br>Center patches sampled: ${num(sum.patchCount,0)} · graphic patches: ${num(sum.graphicPatchCount,0)} (${pct(sum.graphicPatchShare)})<br>Different-from-both-edges: ${pct(sum.differentPatchShare)} · shadow-only: ${pct(sum.shadowOnlyShare)} · coherent: ${sum.coherent?'yes':'no'}<br><br><strong>Structure-first settings</strong><br>Stripe repetition threshold: ${num(settings.stripeMinScore,2)} · Plaid axis threshold: ${num(settings.plaidMinAxis,2)} · Plaid coverage: ${pct(settings.plaidMinCoverage)}<br><br><strong>Broad spatial evidence</strong><br>Left edge range: ${pct(z.left?.range)} · Right edge range: ${pct(z.right?.range)} · Edges match: ${z.edgesMatch?'yes':'no'}<br>Center contrast: ${pct(z.center?.share)} · Vertical transitions: ${num(z.vertical?.count,1)} · Bottom transitions: ${num(z.bottom?.count,1)}<br>Combined contrast area: ${pct(conn.totalShare)} · Largest connected contrast: ${pct(conn.largestShare)}<br><br><strong>Testing:</strong> ${esc(report?.catalog?.label||'selected catalog photo')}</div>`;
  }
  async function run(){
  ...
  console.info(`Audrey Smart Scan ${VERSION} loaded: Calibration Lab includes hybrid structure-first comparison.`);
})();
