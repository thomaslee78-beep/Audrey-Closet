...
(function(){
  ...
  boardItemContent=function(b){if(b?.kind==='text'){normalizeTextPieceV132011(b);const c=textStyleCSSV132011(b.textStyle);return '<div class=\"board-text\" style=\"font-family:'+c.fontFamily+';font-size:'+c.fontSize+';font-weight:'+c.fontWeight+';font-style:'+c.fontStyle+';text-decoration:'+c.textDecoration+';color:'+c.color+'\">'+esc(b.value||'')+'</div>'}return originalBoardItemContentV132011.apply(this,arguments)};
  const originalRenderSnapshotPieceV132011=renderSnapshotPiece;
  renderSnapshotPiece=function(board,p,scaleX,scaleY,offsetX=0,offsetY=0){const before=board?.children?.length||0,result=originalRenderSnapshotPieceV132011.apply(this,arguments);if(p?.kind==='text'&&board){const el=board.children[before]||board.lastElementChild,t=el?.querySelector?.('.board-text');if(t){const c=textStyleCSSV132011(p.textStyle,Math.min(scaleX||1,scaleY||1));Object.assign(t.style,{fontFamily:c.fontFamily,fontSize:c.fontSize,fontWeight:c.fontWeight,fontStyle:c.fontStyle,textDecoration:c.textDecoration,color:c.color})}}return result};
