...
function borderColor(data,w,h){const band=Math.max(2,Math.round(Math.min(w,h)*.035)),step=Math.max(1,Math.round(Math.min(w,h)/220));let r=0,g=0,b=0,n=0;for(let y=0;y<h;y+=step)for(let x=0;x<w;x+=step){if(x>=band&&x<w-band&&y>=band&&y<h-band)continue;const i=(y*w+x)*4;r+=data[i];g+=data[i+1];b+=data[i+2];n++;}return n?[r/n,g/n,b/n]:[255,255,255];}
...
function insidePolygon(local,points){let hit=false;for(let i=0,j=points.length-1;i<points.length;j=i++){const a=points[i],b=points[j];if(((a[1]>local.y)!==(b[1]>local.y))&&(local.x<(b[0]-a[0])*(local.y-a[1])/((b[1]-a[1])||1e-6)+a[0]))hit=!hit;}return hit;}
...
async function protectPolygonGuide({guide,mode}){
  ...
  const original=await sourceToStudioCanvas(src),w=studioBaseCanvas.width,h=studioBaseCanvas.height;if(!w||!h)return{changed:false,rescued:0,insideCount:0};
  ...
  const cutCtx=studioBaseCanvas.getContext('2d',{willReadFrequently:true}),srcImage=sourceCtx.getImageData(0,0,w,h),cutImage=cutCtx.getImageData(0,0,w,h),s=srcImage.data,c=cutImage.data,bg=borderColor(s,w,h),pr=clamp(Number(guide.protection)||70,0,100)/100;
  const threshold=.53-pr*.17+(mode==='clean'?.015:0),floor=30-pr*12,alpha=(x,y)=>c[(y*w+x)*4+3],tr=guide.transform||{},gw=Math.max(1,Number(tr.width)||330),gh=Math.max(1,Number(tr.height)||430);let rescued=0,insideCount=0;
  for(let y=2;y<h-2;y++)for(let x=2;x<w-2;x++){
    const world=imagePointToWorld(x,y),local=guideLocalPoint(world,guide);if(!insidePolygon(local,guide.points))continue;insideCount++;const i=(y*w+x)*4;if(c[i+3]>=230)continue;
    const dr=s[i]-bg[0],dg=s[i+1]-bg[1],db=s[i+2]-bg[2],dist=Math.sqrt(dr*dr+dg*dg+db*db);let strong=0,near=0;
    for(let yy=-2;yy<=2;yy++)for(let xx=-2;xx<=2;xx++){if(!xx&&!yy)continue;const a=alpha(x+xx,y+yy);if(a>35)near++;if(a>150)strong++;}
    const gx=(local.x-.5)*gw,gy=(local.y-.5)*gh,nx=Math.abs(gx/(gw/2)),ny=Math.abs(gy/(gh/2)),central=Math.max(0,1-Math.max(nx,ny)),continuity=Math.min(1,strong/5),color=Math.min(1,dist/66),score=central*(.28+.10*pr)+continuity*.40+color*(.32-.10*pr);
    if(dist<floor&&strong<(pr>.75?2:3))continue;if(score<threshold&&near<(pr>.75?2:3))continue;
    const restoredAlpha=Math.round(Math.max(90,Math.min(250,95+pr*36+continuity*76+color*34+central*20)));c[i]=s[i];c[i+1]=s[i+1];c[i+2]=s[i+2];c[i+3]=Math.max(c[i+3],restoredAlpha);rescued++;
  }
  if(rescued)cutCtx.putImageData(cutImage,0,0);return{changed:rescued>0,rescued,insideCount,protection:Math.round(pr*100)};
}
...
applyStudioMode=async function(mode,options){
  ...
  try{
    ...
    if(result.applied){const status=document.getElementById('studioStatus');if(status){const methodLabels={standard:'Standard',center:'Center Focus',edge:'Edge Guide',grow:'Subject Grow'},modeLabel=mode==='clean'?'Clean':'Quick';status.textContent=modeLabel+' + '+(methodLabels[result.lastState?.method]||'Standard')+' + Shirt Guide applied.';}window.__audreyCutoutWorkflow3B?.sync?.();}
...
window.__audreyCutoutPipeline={phase:'3C',registerGuideProcessor,applyGuideProtection,applyCurrentGuide,getProcessorTypes:()=>[...processors.keys()],get lastStats(){return lastStats?clone(lastStats):null;}};
})();
