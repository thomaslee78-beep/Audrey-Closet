/* Audrey Closet v13.23.5 — production release bootstrap
 * Loads high-fidelity Share, shared Text Layout, accepted Decorate/Draw/Focus
 * runtime, Sticker compatibility, and Share modal scroll locking.
 */
(function(){
  'use strict';

  const HOTFIX='13.23.5-text-layout';
  ...
  function warmReleaseCache(){if(!('caches' in window))return Promise.resolve();return caches.open('audrey-closet-v13.22-dev26').then(cache=>Promise.allSettled(releaseAssets.map(asset=>cache.add(asset)))).catch(err=>console.warn('Audrey v13.23.5 release cache warm skipped',err));}
  function loadSequentially(){if(window.__audreyReleaseV1323Bootstrapped)return;window.__audreyReleaseV1323Bootstrapped=true;let chain=Promise.resolve();modules.forEach(src=>{chain=chain.then(()=>new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src;s.async=false;s.onload=resolve;s.onerror=()=>reject(new Error('Failed to load '+src));document.body.appendChild(s);}));});chain.then(warmReleaseCache).catch(err=>console.error('Audrey v13.23.5 release bootstrap failed',err));}
