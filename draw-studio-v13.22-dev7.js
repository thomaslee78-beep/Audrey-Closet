/* Audrey Closet v13.22 Draw Studio dev7
 * Live freeform tools: Pencil, Pen, Sharpie, Highlighter.
 * Keeps Draw mode constrained to Board -> Decorate -> Draw.
 * Line, Arrow, and Eraser remain UI-only.
 */
(function(){
  'use strict';

  const ROOT_ID='drawStudioDev7';
  const STYLE_ID='drawStudioDev7Styles';
  const TOOLS=[
    ['line','Line','╱'],
    ['arrow','Arrow','➜'],
    ['pencil','Pencil','✎'],
  ...
  const DEFAULT_OPACITY={pencil:1,pen:1,sharpie:1,highlighter:.38};
  ...
  function toolOpacity(tool){return DEFAULT_OPACITY[tool]??1;}
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      #outfitBoard.draw-dev7-active{touch-action:none!important;cursor:crosshair}
      #outfitBoard .draw-live-overlay-dev7{position:absolute;inset:0;width:100%;height:100%;z-index:2147483000;pointer-events:none;overflow:visible}
      .board-piece .drawing-svg-dev7{display:block;width:100%;height:100%;overflow:visible;pointer-events:none}
  ...
  function installDrawingRenderer(){
    ...
    boardItemContent=function(b){
      if(b&&b.kind==='drawing'&&b.drawingType==='freeform'){
        ...
        return `<svg class="drawing-svg-dev7" viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" aria-hidden="true"><polyline points="${esc(b.points||'')}" fill="none" stroke="${color}" stroke-width="${width}" stroke-linecap="round" stroke-linejoin="round"${dash?` stroke-dasharray="${dash}"`:''} opacity="${opacity}" vector-effect="non-scaling-stroke"/></svg>`;
      }
    ...
    normalizeBoardItem=function(b){
      ...
      if(b&&b.kind==='drawing'){
        ...
        b.strokeWidth=Math.max(1,Number(b.strokeWidth)||DEFAULT_THICKNESS[b.tool]||2);
  ...
  function ensureOverlay(board){
    let svg=board.querySelector('.draw-live-overlay-dev7');
    if(svg)svg.remove();
    svg=document.createElementNS('http://www.w3.org/2000/svg','svg');
    svg.setAttribute('class','draw-live-overlay-dev7');
  ...
  function finishStroke(board,e,cancelled){
    ...
    boardItems.push({uid:id(),kind:'drawing',drawingType:'freeform',tool:stroke.tool,strokeColor:stroke.color,strokeWidth:stroke.thickness,strokeStyle:stroke.lineStyle,opacity:stroke.opacity,points:relative,x:minX,y:minY,w,h,rotation:0,z:nextZ()});
  ...
  function applyDrawMode(){
    ...
    const board=document.getElementById('outfitBoard');
    if(board)board.classList.toggle('draw-dev7-active',active);
  ...
  function sync(root){
    ...
    const slider=root.querySelector('#drawThicknessDev7');
    const value=root.querySelector('.draw-thickness-value');
    const color=root.querySelector('#drawColorDev7');
    ...
    if(hint){
      if(live)hint.textContent=state.drawMode&&drawTabIsActive()?'Draw continuously on the board · tap Done to select objects':'Select this tool in Draw to begin';
      else hint.textContent='This tool is UI-only in dev7';
    }
    applyDrawMode();
  }
  ...
  function buildRoot(){
    ...
    root.id=ROOT_ID;
    root.innerHTML=`
      <div class="draw-clean-head"><span>Draw Studio</span><small>dev7 · Freeform tools</small></div>
      ...
      <div class="draw-control-row">
        <label class="draw-thickness"><span class="draw-control-label">Size</span><input id="drawThicknessDev7" type="range" min="1" max="24" step="1" value="2" aria-label="Draw thickness"><span class="draw-thickness-value">2</span></label>
        <label class="draw-color-wrap" aria-label="Draw color"><input id="drawColorDev7" type="color" value="#4f514a"></label>
    ...
    root.querySelector('#drawThicknessDev7')?.addEventListener('input',e=>{state.thickness=Math.max(1,Math.min(24,Number(e.target.value)||1));sync(root);});
    root.querySelector('#drawColorDev7')?.addEventListener('input',e=>{state.color=e.target.value||'#4f514a';sync(root);});
