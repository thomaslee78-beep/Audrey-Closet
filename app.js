...
let portfolioDrag={timer:null,pointerId:null,startX:0,startY:0,x:0,y:0,card:null,folder:'',active:false,ghost:null,dropOutline:null,targetId:null,targetAfter:false,originalId:null,raf:null};
...
async function init(){
  ...
  if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=13.11-dev4',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{});
  renderAll();
}
...
function resetPortfolioDrag(){clearTimeout(portfolioDrag.timer);if(portfolioDrag.raf)cancelAnimationFrame(portfolioDrag.raf);portfolioDrag.ghost?.remove();portfolioDrag.dropOutline?.remove();$$('#savedOutfits .portfolio-drop-target').forEach(x=>x.classList.remove('portfolio-drop-target','drop-before','drop-after'));portfolioDrag.card?.classList.remove('portfolio-drag-source');document.body.classList.remove('portfolio-reordering');portfolioDrag={timer:null,pointerId:null,startX:0,startY:0,x:0,y:0,card:null,folder:'',active:false,ghost:null,dropOutline:null,targetId:null,targetAfter:false,originalId:null,raf:null} }
function beginPortfolioPress(card,e){if(portfolioFilter==='All'||portfolioFilter==='Favorites')return;resetPortfolioDrag();portfolioDrag={...portfolioDrag,card,folder:portfolioFilter,pointerId:e.pointerId,startX:e.clientX,startY:e.clientY,x:e.clientX,y:e.clientY};portfolioDrag.timer=setTimeout(()=>startPortfolioDrag(),430)}
function startPortfolioDrag(){const d=portfolioDrag;if(!d.card||!d.folder)return;d.timer=null;d.active=true;d.originalId=d.card.dataset.id;d.targetId=null;suppressPortfolioClickUntil=Date.now()+900;const r=d.card.getBoundingClientRect();d.card.classList.add('portfolio-drag-source');const ghost=d.card.cloneNode(true);ghost.classList.add('portfolio-drag-ghost');ghost.removeAttribute('data-id');ghost.querySelectorAll('button').forEach(b=>b.disabled=true);ghost.querySelectorAll('img').forEach(img=>{img.draggable=false});Object.assign(ghost.style,{position:'fixed',left:'0',top:'0',width:r.width+'px',height:r.height+'px',margin:'0',pointerEvents:'none',zIndex:'5000'});document.body.appendChild(ghost);d.ghost=ghost;d.ghostOffsetX=d.x-r.left;d.ghostOffsetY=d.y-r.top;const outline=document.createElement('div');outline.className='portfolio-drop-outline';document.body.appendChild(outline);d.dropOutline=outline;document.body.classList.add('portfolio-reordering');navigator.vibrate?.(18);positionPortfolioGhost(d.x,d.y);toast('Reorder mode — drag this look to a new position')}
...
function updatePortfolioDropTarget(x,y){const d=portfolioDrag,grid=$('#savedOutfits');if(!grid||!d.card)return;const cards=[...grid.querySelectorAll('.portfolio-card[data-id]')].filter(c=>c!==d.card);clearPortfolioDropTarget();if(!cards.length)return;let target=null,best=Infinity;for(const c of cards){const r=c.getBoundingClientRect(),dist=Math.hypot(x-(r.left+r.width/2),y-(r.top+r.height/2));if(dist<best){best=dist;target=c}}if(!target)return;const r=target.getBoundingClientRect(),sameRow=y>=r.top-r.height*.2&&y<=r.bottom+r.height*.2,after=sameRow?x>r.left+r.width/2:y>r.top+r.height/2;d.targetId=target.dataset.id;d.targetAfter=after;target.classList.add('portfolio-drop-target',after?'drop-after':'drop-before');if(d.dropOutline){Object.assign(d.dropOutline.style,{left:r.left+'px',top:r.top+'px',width:r.width+'px',height:r.height+'px'});d.dropOutline.classList.add('visible')}}
function movePortfolioDrag(e){const d=portfolioDrag;if(!d.card||d.pointerId!==e.pointerId)return;d.x=e.clientX;d.y=e.clientY;const dist=Math.hypot(d.x-d.startX,d.y-d.startY);if(!d.active){if(dist>14){clearTimeout(d.timer);d.timer=null}return}if(e.cancelable)e.preventDefault();positionPortfolioGhost(d.x,d.y);if(dist<24){clearPortfolioDropTarget();d.targetId=null;return}if(!d.raf)d.raf=requestAnimationFrame(()=>{d.raf=null;const edge=105,speed=10;if(d.y<edge)window.scrollBy(0,-speed);else if(d.y>window.innerHeight-edge)window.scrollBy(0,speed);updatePortfolioDropTarget(d.x,d.y)})}
async function finishPortfolioDrag(e,cancelled=false){const d=portfolioDrag;if(!d.card||d.pointerId!==e.pointerId)return;clearTimeout(d.timer);if(d.active){if(e.cancelable)e.preventDefault();suppressPortfolioClickUntil=Date.now()+650;if(!cancelled&&d.targetId&&d.originalId){ensureSettings();const folderIds=state.outfits.filter(o=>(o.folder||'Everyday')===d.folder).map(o=>o.id);let order=(state.settings.portfolioOrder[d.folder]||[]).filter(id=>folderIds.includes(id));order=[...order,...folderIds.filter(id=>!order.includes(id))].filter(id=>id!==d.originalId);let idx=order.indexOf(d.targetId);if(idx<0)idx=0;if(d.targetAfter)idx++;order.splice(Math.max(0,Math.min(order.length,idx)),0,d.originalId);state.settings.portfolioOrder[d.folder]=[...new Set(order)];await saveState();renderSavedOutfits();toast('Portfolio order saved')}}resetPortfolioDrag()}
function bindPortfolioReorder(){const enabled=portfolioFilter!=='All'&&portfolioFilter!=='Favorites';$$('#savedOutfits .portfolio-card').forEach(card=>{card.onpointerdown=e=>{if(!enabled||e.button!==0||e.target.closest('.favorite-outfit'))return;beginPortfolioPress(card,e);try{card.setPointerCapture(e.pointerId)}catch{}};card.onpointermove=movePortfolioDrag;card.onpointerup=e=>finishPortfolioDrag(e,false);card.onpointercancel=e=>finishPortfolioDrag(e,true);card.oncontextmenu=e=>{if(portfolioDrag.active||portfolioDrag.timer)e.preventDefault()}});const hint=$('#portfolioReorderHint');if(hint)hint.textContent=enabled?'Press and hold a look to reorder this folder.':'Choose a portfolio folder to reorder looks.'}
function renderSavedOutfits(){
  ...
  const shown=portfolioFilter==='All'?state.outfits.slice():portfolioFilter==='Favorites'?state.outfits.filter(o=>o.favorite):orderedOutfitsForFolder(portfolioFilter);
...
async function deletePortfolioFolder(index){if(state.settings.portfolioFolders.length<=1)return;const old=state.settings.portfolioFolders[index],replacement=state.settings.portfolioFolders.find((_,i)=>i!==index)||'Everyday';if(!confirm(`Remove “${old}”? Saved looks in it will move to “${replacement}”.`))return;const moved=(state.settings.portfolioOrder[old]||[]).slice();state.settings.portfolioFolders.splice(index,1);state.outfits.forEach(o=>{if((o.folder||'Everyday')===old)o.folder=replacement});state.settings.portfolioOrder[replacement]=[...(state.settings.portfolioOrder[replacement]||[]),...moved.filter(x=>!(state.settings.portfolioOrder[replacement]||[]).includes(x))];delete state.settings.portfolioOrder[old];if(portfolioFilter===old)portfolioFilter='All';await saveState();populatePortfolioFolderSelect(replacement);renderPortfolioFolderEditor();toast('Folder removed')}
