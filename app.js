...
async function init(){
  ...
  if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=13.8-dev1.8',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{});
  renderAll();
}
...
function cloneStudioHistoryState(){return{erase:cloneCanvasImageData(studioManualEraseMask),restore:cloneCanvasImageData(studioManualRestoreMask)}}
function restoreStudioHistoryState(s){if(!s)return;restoreCanvasImageData(studioManualEraseMask,s.erase);restoreCanvasImageData(studioManualRestoreMask,s.restore);rebuildStudioWorkCanvas()}
...
function resetStudioHistory(){studioUndoStack=[];studioRedoStack=[];updateStudioHistoryButtons()}
function pushStudioHistory(){const s=cloneStudioHistoryState();if(!s.erase||!s.restore)return;studioUndoStack.push(s);if(studioUndoStack.length>12)studioUndoStack.shift();studioRedoStack=[];updateStudioHistoryButtons()}
...
function toggleStudioMove(){studioMoveMode=!studioMoveMode;studioBrushMode=null;studioDrawing=false;studioGesture=null;studioPointers.clear();if(studioMoveMode){studioViewZoom=1;studioViewX=0;studioViewY=0;updateStudioZoomLabel();renderStudio()}updateStudioToolUI();$('#studioStatus').textContent=studioMoveMode?'Move mode: view reset to 100% so the dashed guide matches the saved photo. Drag to reposition; pinch to resize and gently twist to rotate. Tap Move again to lock it.':t('studio.noTool')}
...
async function removeSimpleBackground(dataURL,edge=45){const img=await imageFrom(dataURL),max=900,scale=Math.min(1,max/Math.max(img.width,img.height)),c=document.createElement('canvas');c.width=Math.round(img.width*scale);c.height=Math.round(img.height*scale);const ctx=c.getContext('2d');ctx.drawImage(img,0,0,c.width,c.height);const im=ctx.getImageData(0,0,c.width,c.height),d=im.data,w=c.width,h=c.height;const samples=[];const take=(x,y)=>{const i=(y*w+x)*4;samples.push([d[i],d[i+1],d[i+2]])};for(let x=0;x<w;x+=Math.max(1,Math.floor(w/30))){take(x,0);take(x,h-1)}for(let y=0;y<h;y+=Math.max(1,Math.floor(h/30))){take(0,y);take(w-1,y)}const bg=samples.reduce((a,p)=>[a[0]+p[0],a[1]+p[1],a[2]+p[2]],[0,0,0]).map(x=>x/samples.length),low=Math.max(18,edge*.75),high=Math.max(low+28,edge*1.6);for(let i=0;i<d.length;i+=4){const dist=Math.sqrt((d[i]-bg[0])**2+(d[i+1]-bg[1])**2+(d[i+2]-bg[2])**2);if(dist<low)d[i+3]=0;else if(dist<high)d[i+3]=Math.round(255*(dist-low)/(high-low))}ctx.putImageData(im,0,0);return c.toDataURL('image/png')}
