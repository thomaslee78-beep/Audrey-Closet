...
let portfolioDrag={timer:null,pointerId:null,touchId:null,startX:0,startY:0,x:0,y:0,card:null,folder:'',active:false,moved:false,ghost:null,dropOutline:null,targetId:null,targetAfter:false,originalId:null,raf:null,scrollRaf:null,aggregate:false};
...
async function init(){
  ...
  if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=13.11-dev4.1',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{});
  renderAll();
}
...
function resetPortfolioDrag(){const d=portfolioDrag;clearTimeout(d.timer);if(d.raf)cancelAnimationFrame(d.raf);if(d.scrollRaf)cancelAnimationFrame(d.scrollRaf);unbindActivePortfolioTouchTracking();d.ghost?.remove();d.dropOutline?.remove();$$('#savedOutfits .portfolio-drop-target').forEach(x=>x.classList.remove('portfolio-drop-target','drop-before','drop-after'));d.card?.classList.remove('portfolio-drag-source');document.body.classList.remove('portfolio-reordering');portfolioDrag={timer:null,pointerId:null,touchId:null,startX:0,startY:0,x:0,y:0,card:null,folder:'',active:false,moved:false,ghost:null,dropOutline:null,targetId:null,targetAfter:false,originalId:null,raf:null,scrollRaf:null,aggregate:false}}
function beginPortfolioPress(card,folder,pointerId,x,y,touchId=null){resetPortfolioDrag();const aggregate=folder==='All'||folder==='Favorites';portfolioDrag={...portfolioDrag,card,folder:aggregate?'':folder,pointerId,touchId,startX:x,startY:y,x,y,aggregate};portfolioDrag.timer=setTimeout(()=>{if(portfolioDrag.aggregate){portfolioDrag.timer=null;suppressPortfolioClickUntil=Date.now()+800;navigator.vibrate?.(12);toast('Choose a specific portfolio category before reordering looks.');return}startPortfolioDrag()},430)}
