...
async function init(){
  ...
  if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=13.9-dev1.1',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{});
  renderAll();
}
...
function buildCutoutPass(im,w,h,edge=45,mode='quick'){const d=im.data;const {bg,mad}=borderBackgroundModel(d,w,h);const passable=new Uint8Array(w*h),strength=new Uint8ClampedArray(w*h);const base=mode==='clean'?0.82:0.74,gradLimit=(mode==='clean'?26:21)+edge*.28,floodThresh=Math.max(18,edge*base+mad*.55),sureFg=floodThresh+(mode==='clean'?20:15);for(let y=0;y<h;y++)for(let x=0;x<w;x++){const p=y*w+x,i=p*4,r=d[i],g=d[i+1],b=d[i+2],dr=r-bg[0],dg=g-bg[1],db=b-bg[2],dist=Math.sqrt(dr*dr+dg*dg+db*db),xr=Math.min(w-1,x+1),yd=Math.min(h-1,y+1),ir=(y*w+xr)*4,id=(yd*w+x)*4,grad=Math.max(Math.abs(r-d[ir])+Math.abs(g-d[ir+1])+Math.abs(b-d[ir+2]),Math.abs(r-d[id])+Math.abs(g-d[id+1])+Math.abs(b-d[id+2]))/3;passable[p]=(dist<floodThresh&&grad<gradLimit)?1:0;strength[p]=clamp(Math.round((dist-floodThresh)/(sureFg-floodThresh)*255),0,255)}const bgFlood=new Uint8Array(w*h),q=new Int32Array(w*h);let head=0,tail=0;const push=p=>{if(p<0||p>=w*h||bgFlood[p]||!passable[p])return;bgFlood[p]=1;q[tail++]=p};for(let x=0;x<w;x++){push(x);push((h-1)*w+x)}for(let y=0;y<h;y++){push(y*w);push(y*w+w-1)}while(head<tail){const p=q[head++],x=p%w,y=(p/w)|0;if(x>0)push(p-1);if(x<w-1)push(p+1);if(y>0)push(p-w);if(y<h-1)push(p+w)}let mask=new Uint8Array(w*h);for(let p=0;p<w*h;p++)mask[p]=bgFlood[p]?0:1;for(let p=0;p<w*h;p++)if(strength[p]>=(mode==='clean'?230:215))mask[p]=1;mask=fillMaskHoles(mask,w,h);mask=keepMainForeground(mask,w,h,mode==='clean'?.10:.07,mode==='clean'?.035:.02);mask=closeMask(mask,w,h);const alpha=new Uint8ClampedArray(w*h);for(let p=0;p<w*h;p++){if(!mask[p]){alpha[p]=0;continue}alpha[p]=isMaskBoundary(mask,w,h,p)?Math.max(mode==='clean'?215:200,strength[p]):255}return {alpha,mask}}
