...
async function init(){
  ...
  if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=13.12-dev1.3.2',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{});
  renderAll();
}
...
function resetItemSwipeVisual(zone,{animate=false}={}){if(!zone)return;zone.classList.toggle('swipe-snap-back',animate);zone.classList.remove('swipe-dragging');zone.style.setProperty('--item-swipe-x','0px');zone.style.opacity='';if(animate)setTimeout(()=>zone.classList.remove('swipe-snap-back'),170)}
function bindItemSwipe(){
  ...
  zone.addEventListener('touchmove',e=>{if(!itemSwipeStart||!$('#itemId').value||e.touches.length!==1)return;const t=e.touches[0],dx=t.clientX-itemSwipeStart.x,dy=t.clientY-itemSwipeStart.y;if(!itemSwipeStart.axis&&Math.hypot(dx,dy)>10){if(Math.abs(dx)>Math.abs(dy)*1.15)itemSwipeStart.axis='x';else if(Math.abs(dy)>Math.abs(dx)*1.05)itemSwipeStart.axis='y'}if(itemSwipeStart.axis!=='x')return;e.preventDefault();const visualX=Math.max(-74,Math.min(74,dx*.58));zone.classList.add('swipe-dragging');zone.style.setProperty('--item-swipe-x',`${visualX}px`);zone.style.opacity=String(Math.max(.78,1-Math.abs(visualX)/310))},{passive:false});
  zone.addEventListener('touchend',e=>{if(!itemSwipeStart||!$('#itemId').value)return;const start=itemSwipeStart,t=e.changedTouches[0],dx=t.clientX-start.x,dy=t.clientY-start.y,elapsed=Date.now()-start.time;itemSwipeStart=null;if(start.axis!=='x'||elapsed>850||Math.abs(dx)<55||Math.abs(dx)<Math.abs(dy)*1.15){resetItemSwipeVisual(zone,{animate:start.axis==='x'});return}navigateReviewItem(dx<0?1:-1)});
  zone.addEventListener('touchcancel',()=>{itemSwipeStart=null;resetItemSwipeVisual(zone,{animate:true})},{passive:true});
}
function navigateReviewItem(delta){
  const current=$('#itemId').value;if(!current||itemReviewIds.length<2)return;const idx=itemReviewIds.indexOf(current);if(idx<0)return;const nextIndex=idx+delta,zone=$('#itemSwipeZone');if(nextIndex<0||nextIndex>=itemReviewIds.length){resetItemSwipeVisual(zone,{animate:true});toast(delta<0?'First piece in this view':'Last piece in this view');return}const next=state.items.find(i=>i.id===itemReviewIds[nextIndex]);if(!next)return;zone.classList.remove('swipe-dragging','swipe-snap-back','swipe-out-next','swipe-out-prev','swipe-in-next','swipe-in-prev');zone.style.setProperty('--item-swipe-x','0px');zone.style.opacity='';void zone.offsetWidth;zone.classList.add(delta>0?'swipe-out-next':'swipe-out-prev');setTimeout(()=>{loadItemIntoEditor(next);zone.classList.remove('swipe-out-next','swipe-out-prev');void zone.offsetWidth;zone.classList.add(delta>0?'swipe-in-next':'swipe-in-prev');setTimeout(()=>zone.classList.remove('swipe-in-next','swipe-in-prev'),190)},125);
}
...
function updateReviewPhotoMenuState(){const restore=$('#reviewRestoreOriginalBtn');if(restore)restore.classList.toggle('hidden',!itemOriginalPhoto||itemWorkingPhoto===itemOriginalPhoto);const scan=$('#smartScanIconBtn');if(scan)scan.classList.toggle('hidden',!itemWorkingPhoto);const utilities=$('#itemCardUtilityActions');if(utilities)utilities.classList.toggle('hidden',!$('#itemId').value&&!itemWorkingPhoto)}
function updatePhotoToolAvailability(){const studio=$('#photoStudioBtn');if(studio){studio.disabled=!itemWorkingPhoto;studio.classList.toggle('hidden',!itemWorkingPhoto)}const reviewStudio=$('#reviewStudioBtn');if(reviewStudio)reviewStudio.disabled=!itemWorkingPhoto}
...
async function smartScan(){if(!itemWorkingPhoto)return toast('Take or choose a photo first');setPhotoBusy(true,'Scanning color, pattern and visible text…');try{const visual=await analyzeImage(itemWorkingPhoto);let ocr='';try{ocr=await tryOCR(itemWorkingPhoto)}catch{}const flat=ocr.replace(/\n/g,' ');const brands=['Nike','Adidas','Lacoste','Gap','Old Navy','Zara','H&M','Uniqlo','Levi','Levi\'s','Converse','Vans','Champion','Aritzia','Brandy Melville','Hollister','Abercrombie','American Eagle','Puma','New Balance','Patagonia','North Face'];const brand=brands.find(b=>new RegExp(`\b${b.replace("'","\\'")}\b`,'i').test(flat))||'';const sm=flat.match(/\b(XXS|XS|S|M|L|XL|XXL|[0-9]{1,2}(?:\.[05])?)\b/i);pendingSmartScanResult={color:visual.color||'',pattern:visual.pattern||'',brand,size:sm?sm[1].toUpperCase():''};openSmartScanReview(pendingSmartScanResult);$('#scanStatus').textContent='Smart Scan complete. Review detected details before applying.'}catch(err){console.error(err);toast('Smart Scan could not analyze this photo');$('#scanStatus').textContent='Smart Scan could not analyze this photo.'}finally{setPhotoBusy(false)}}
...
function applyPendingSmartScan(){if(!pendingSmartScanResult)return closeSmartScanReview();const chosen=new Set($$('#smartScanReviewFields input[data-scan-field]:checked').map(x=>x.dataset.scanField));if(chosen.has('color')&&pendingSmartScanResult.color)$('#itemColor').value=pendingSmartScanResult.color;if(chosen.has('pattern')&&pendingSmartScanResult.pattern)$('#itemPattern').value=pendingSmartScanResult.pattern;if(chosen.has('brand')&&pendingSmartScanResult.brand)$('#itemBrand').value=pendingSmartScanResult.brand;if(chosen.has('size')&&pendingSmartScanResult.size){const opt=$$('#itemSize option').find(o=>o.value===pendingSmartScanResult.size||o.textContent===pendingSmartScanResult.size);if(opt)$('#itemSize').value=opt.value}updateItemReviewSummary();$('#scanStatus').textContent='Selected Smart Scan details applied. Review them before saving.';closeSmartScanReview();toast('Detected details applied')}
