...
let portfolioTabDrag={timer:null,touchId:null,pointerId:null,active:false,source:null,ghost:null,target:null,startX:0,startY:0,x:0,y:0};
...
async function init(){
  ...
  $('#portfolioSearch').oninput=e=>{portfolioSearchQuery=e.target.value||'';renderSavedOutfits()};$('#portfolioSearch').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();e.currentTarget.blur()}};$('#portfolioSearch').onsearch=e=>{portfolioSearchQuery=e.target.value||'';renderSavedOutfits();e.currentTarget.blur()};$('#portfolioItemFilterBtn').onclick=()=>{portfolioItemPickerOpen=!portfolioItemPickerOpen;renderPortfolioDiscovery()};$('#portfolioClearFilters').onclick=clearPortfolioDiscovery;
  if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=13.11-dev6.1',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{});
  renderAll();
}
...
function closeItemWithoutSaving(){
  ...
  itemWorkingPhoto='';itemOriginalPhoto='';itemCutoutApplied=false;itemStudioState=null;itemPhotoPickerActive=false;$('#itemPhoto').value='';$('#itemPhotoLibrary').value='';$('#scanStatus').textContent='';
...
async function permanentDeleteItem(){
  const iid=$('#itemId').value,item=state.items.find(x=>x.id===iid);if(!item)return;
  const label=item.type||item.category||'this piece';
...
function renderPieceTray(){
  ensureSettings();
  const all=traySource==='closet'?state.items.filter(i=>!isArchived(i)):state.wishlist;
...
function resetPortfolioTabDrag(){const d=portfolioTabDrag;clearTimeout(d.timer);d.ghost?.remove();$$('#portfolioTabs .portfolio-tab-drop-target').forEach(x=>x.classList.remove('portfolio-tab-drop-target'));d.source?.classList.remove('portfolio-tab-drag-source');document.body.classList.remove('portfolio-tab-reordering');portfolioTabDrag={timer:null,touchId:null,pointerId:null,active:false,source:null,ghost:null,target:null,startX:0,startY:0,x:0,y:0}}
function startPortfolioTabDrag(){const d=portfolioTabDrag;if(!d.source)return;d.active=true;suppressPortfolioTabClickUntil=Date.now()+1200;const r=d.source.getBoundingClientRect(),g=d.source.cloneNode(true);g.classList.add('portfolio-tab-drag-ghost');Object.assign(g.style,{position:'fixed',left:r.left+'px',top:r.top+'px',width:r.width+'px',height:r.height+'px',margin:'0',pointerEvents:'none',zIndex:'6000'});document.body.appendChild(g);d.ghost=g;d.offsetX=d.x-r.left;d.offsetY=d.y-r.top;d.source.classList.add('portfolio-tab-drag-source');document.body.classList.add('portfolio-tab-reordering');navigator.vibrate?.(12);positionPortfolioTabGhost(d.x,d.y)}
function positionPortfolioTabGhost(x,y){const d=portfolioTabDrag;if(!d.active||!d.ghost)return;d.ghost.style.left=(x-d.offsetX)+'px';d.ghost.style.top=(y-d.offsetY)+'px';const el=document.elementFromPoint(x,y)?.closest?.('#portfolioTabs .portfolio-tab');if(el&&el!==d.source){d.target?.classList.remove('portfolio-tab-drop-target');d.target=el;el.classList.add('portfolio-tab-drop-target')}else if(!el){d.target?.classList.remove('portfolio-tab-drop-target');d.target=null}}
async function finishPortfolioTabDrag(){const d=portfolioTabDrag;if(!d.active){resetPortfolioTabDrag();return}const sourceName=d.source?.dataset.portfolio,targetName=d.target?.dataset.portfolio;if(sourceName&&targetName&&sourceName!==targetName){ensureSettings();const order=state.settings.portfolioTabOrder,from=order.indexOf(sourceName),to=order.indexOf(targetName);if(from>=0&&to>=0){order.splice(from,1);order.splice(to,0,sourceName);await saveState()}}suppressPortfolioTabClickUntil=Date.now()+700;resetPortfolioTabDrag();renderSavedOutfits()}
function bindPortfolioTabReorder(){const tabs=$$('#portfolioTabs .portfolio-tab');tabs.forEach(tab=>{tab.oncontextmenu=e=>e.preventDefault();tab.addEventListener('touchstart',e=>{if(e.touches.length!==1)return;const t=e.touches[0];resetPortfolioTabDrag();portfolioTabDrag={...portfolioTabDrag,source:tab,touchId:t.identifier,startX:t.clientX,startY:t.clientY,x:t.clientX,y:t.clientY};portfolioTabDrag.timer=setTimeout(startPortfolioTabDrag,430)},{passive:true});tab.addEventListener('touchmove',e=>{const t=[...e.touches].find(x=>x.identifier===portfolioTabDrag.touchId);if(!t)return;portfolioTabDrag.x=t.clientX;portfolioTabDrag.y=t.clientY;if(!portfolioTabDrag.active&&Math.hypot(t.clientX-portfolioTabDrag.startX,t.clientY-portfolioTabDrag.startY)>10){clearTimeout(portfolioTabDrag.timer);return}if(portfolioTabDrag.active){e.preventDefault();positionPortfolioTabGhost(t.clientX,t.clientY)}},{passive:false});tab.addEventListener('touchend',e=>{if(portfolioTabDrag.source===tab)finishPortfolioTabDrag()});tab.addEventListener('touchcancel',()=>{if(portfolioTabDrag.source===tab)resetPortfolioTabDrag()});tab.addEventListener('pointerdown',e=>{if(e.pointerType==='touch'||e.button!==0)return;resetPortfolioTabDrag();portfolioTabDrag={...portfolioTabDrag,source:tab,pointerId:e.pointerId,startX:e.clientX,startY:e.clientY,x:e.clientX,y:e.clientY};portfolioTabDrag.timer=setTimeout(startPortfolioTabDrag,430)});tab.addEventListener('pointermove',e=>{if(portfolioTabDrag.pointerId!==e.pointerId)return;portfolioTabDrag.x=e.clientX;portfolioTabDrag.y=e.clientY;if(!portfolioTabDrag.active&&Math.hypot(e.clientX-portfolioTabDrag.startX,e.clientY-portfolioTabDrag.startY)>8){clearTimeout(portfolioTabDrag.timer);return}if(portfolioTabDrag.active){e.preventDefault();positionPortfolioTabGhost(e.clientX,e.clientY)}});tab.addEventListener('pointerup',e=>{if(portfolioTabDrag.pointerId===e.pointerId)finishPortfolioTabDrag()});tab.addEventListener('pointercancel',e=>{if(portfolioTabDrag.pointerId===e.pointerId)resetPortfolioTabDrag()})})}
...
function portfolioOutfitMatchesDiscovery(outfit){const selected=[...portfolioDiscoveryItemIds];if(selected.length){const ids=new Set((outfit.pieces||[]).filter(p=>(p.kind||'piece')==='piece'&&(p.source||'closet')==='closet').map(p=>p.id));if(!selected.every(id=>ids.has(id)))return false}const q=portfolioSearchQuery.trim().toLowerCase();if(!q)return true;const itemText=portfolioOutfitItemObjects(outfit).map(({obj})=>[obj.category,obj.type,obj.name,obj.color,obj.brand,obj.size,obj.pattern,obj.season,obj.notes].filter(Boolean).join(' ')).join(' ');return [outfit.name,outfit.notes,outfit.folder,itemText].filter(Boolean).join(' ').toLowerCase().includes(q)}
function portfolioDiscoveryLabel(item){return [item.type||item.category,item.color].filter(Boolean).join(' · ')||'Closet item'}
function renderPortfolioDiscovery(){const search=$('#portfolioSearch');if(search&&search.value!==portfolioSearchQuery)search.value=portfolioSearchQuery;const filterBtn=$('#portfolioItemFilterBtn');if(filterBtn){filterBtn.textContent=portfolioItemPickerOpen?'▦ Closet items ▴':'▦ Closet items ▾';filterBtn.setAttribute('aria-expanded',portfolioItemPickerOpen?'true':'false')}const selectedBox=$('#portfolioSelectedFilters');if(selectedBox){const selected=[...portfolioDiscoveryItemIds].map(id=>state.items.find(i=>i.id===id)).filter(Boolean);selectedBox.innerHTML=selected.map(i=>`<button type="button" class="portfolio-filter-chip" data-filter-item="${i.id}" title="Remove ${esc(portfolioDiscoveryLabel(i))}">${i.photo?`<img src="${i.photo}" alt="">`:''}<span>${esc(portfolioDiscoveryLabel(i))}</span><b>×</b></button>`).join('');selectedBox.classList.toggle('hidden',!selected.length);$$('#portfolioSelectedFilters [data-filter-item]').forEach(b=>b.onclick=()=>{portfolioDiscoveryItemIds.delete(b.dataset.filterItem);renderSavedOutfits()})}const picker=$('#portfolioItemFilterPanel');if(picker){picker.classList.toggle('hidden',!portfolioItemPickerOpen);if(portfolioItemPickerOpen){const active=state.items.filter(i=>!isArchived(i));picker.innerHTML=`<div class="portfolio-filter-picker-head"><div><strong>Filter by closet items</strong><small>Select more than one to find looks containing all selected items.</small></div><button type="button" class="text-btn portfolio-filter-close" id="portfolioItemFilterClose">Close</button></div><div class="portfolio-filter-item-grid">${active.map(i=>`<button type="button" class="portfolio-filter-item ${portfolioDiscoveryItemIds.has(i.id)?'selected':''}" data-pick-item="${i.id}">${i.photo?`<img src="${i.photo}" alt="${esc(i.type||i.category||'Closet item')}">`:'<span class="portfolio-filter-placeholder">✣</span>'}<span>${esc(i.type||i.category||'Piece')}</span><small>${esc([i.color,i.brand].filter(Boolean).join(' · ')||i.category||'')}</small></button>`).join('')||'<p class="portfolio-filter-empty">Add closet pieces to use item filters.</p>'}</div>`;$('#portfolioItemFilterClose').onclick=()=>{portfolioItemPickerOpen=false;renderPortfolioDiscovery()};$$('#portfolioItemFilterPanel [data-pick-item]').forEach(b=>b.onclick=()=>{const id=b.dataset.pickItem;if(portfolioDiscoveryItemIds.has(id))portfolioDiscoveryItemIds.delete(id);else portfolioDiscoveryItemIds.add(id);renderSavedOutfits()})}}const clear=$('#portfolioClearFilters');if(clear)clear.classList.toggle('hidden',!portfolioDiscoveryActive())}
...
function renderSavedOutfits(){
  ...
  $$('#portfolioTabs [data-portfolio]').forEach(b=>b.onclick=()=>{if(Date.now()<suppressPortfolioTabClickUntil)return;portfolioFilter=b.dataset.portfolio;renderSavedOutfits()});bindPortfolioTabReorder();
...
function journalItemDetailRows(item){
  const rows=[['Category',item.category],['Type',item.type],['Brand',item.brand],['Color',item.color],['Size',item.size],['Pattern',item.pattern],['Season',item.season],['Acquired',item.acquired],['Wears',Number(item.wears||0)||0],['Notes',item.notes]].filter(([,value])=>value!==undefined&&value!==null&&String(value).trim()!=='');
...
async function applyPhotoStudio(){const drawCanvas=studioAdjustedCanvas||studioWorkCanvas;if(!drawCanvas)return;const out=newStudioCanvas(),ctx=out.getContext('2d');studioFillBackground(ctx);ctx.save();ctx.translate(360+studioObjectX,360+studioObjectY);ctx.rotate(studioObjectRotation*Math.PI/180);ctx.scale(studioObjectScale,studioObjectScale);ctx.drawImage(drawCanvas,-360,-360);ctx.restore();if(studioBg==='transparent')itemWorkingPhoto=out.toDataURL('image/png');else{itemWorkingPhoto=out.toDataURL('image/webp',.86);if(!itemWorkingPhoto.startsWith('data:image/webp'))itemWorkingPhoto=out.toDataURL('image/jpeg',.88)}itemCutoutApplied=studioMode!=='original'||maskHasPaint(studioManualEraseMask)||maskHasPaint(studioManualRestoreMask)||studioHasAdjustments();itemStudioState=studioLegacyMode?null:{version:3,sourceFingerprint:photoFingerprint(itemOriginalPhoto||''),mode:studioMode,edge:studioEdge,eraseMask:maskDataURL(studioManualEraseMask),restoreMask:maskDataURL(studioManualRestoreMask),scale:studioObjectScale,x:studioObjectX,y:studioObjectY,rotation:studioObjectRotation,exposure:studioExposure,contrast:studioContrast,highlights:studioHighlights,bg:studioBg,customBg:studioCustomBg};showPhoto('#itemPhotoPreview','#photoPlaceholder',itemWorkingPhoto);updateOriginalPhotoButton();$('#photoStudioDialog').close();$('#scanStatus').textContent=studioBg==='transparent'?'Photo Studio image applied · transparency preserved.':'Photo Studio image applied · standardized square crop.';toast('Photo applied')}
