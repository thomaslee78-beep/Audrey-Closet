...
const UI_STRINGS={
  en:{
    ...
    'studio.resetConfirm':'Reset to the captured original and remove cutout, manual masking, position, size and rotation edits?',
...
async function init(){
  ...
  if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=13.9-dev1',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{});
  renderAll();
}
...
function cloneStudioHistoryState(){return{erase:cloneCanvasImageData(studioManualEraseMask),restore:cloneCanvasImageData(studioManualRestoreMask),scale:studioObjectScale,x:studioObjectX,y:studioObjectY,rotation:studioObjectRotation}}
function restoreStudioHistoryState(s){if(!s)return;restoreCanvasImageData(studioManualEraseMask,s.erase);restoreCanvasImageData(studioManualRestoreMask,s.restore);if(Number.isFinite(s.scale))studioObjectScale=s.scale;if(Number.isFinite(s.x))studioObjectX=s.x;if(Number.isFinite(s.y))studioObjectY=s.y;if(Number.isFinite(s.rotation))studioObjectRotation=s.rotation;rebuildStudioWorkCanvas()}
...
function resetStudioHistory(){studioUndoStack=[];studioRedoStack=[];studioPendingTransformHistory=null;updateStudioHistoryButtons()}
...
async function renderStudio(){const c=$('#studioCanvas'),ctx=c.getContext('2d');if(c.width!==720||c.height!==720){c.width=720;c.height=720}ctx.clearRect(0,0,720,720);ctx.fillStyle='#e9e3d8';ctx.fillRect(0,0,720,720);drawStudioBoard(ctx);if(!studioWorkCanvas)return;ctx.save();ctx.translate(360+studioViewX,360+studioViewY);ctx.scale(studioViewZoom,studioViewZoom);ctx.translate(studioObjectX,studioObjectY);ctx.rotate(studioObjectRotation*Math.PI/180);ctx.scale(studioObjectScale,studioObjectScale);ctx.drawImage(studioWorkCanvas,-360,-360);ctx.restore()}
...
function toggleStudioMove(){studioMoveMode=!studioMoveMode;studioBrushMode=null;studioDrawing=false;studioGesture=null;studioPointers.clear();studioPendingTransformHistory=null;if(studioMoveMode){studioViewZoom=1;studioViewX=0;studioViewY=0;updateStudioZoomLabel();renderStudio()}updateStudioToolUI();$('#studioStatus').textContent=studioMoveMode?'Adjust mode: view reset to 100% so the dashed guide matches the saved photo. Drag to reposition; pinch to resize and gently twist to rotate. Tap Adjust again to lock it.':t('studio.noTool')}
...
async function applyPhotoStudio(){if(!studioWorkCanvas)return;const out=newStudioCanvas(),ctx=out.getContext('2d');studioFillBackground(ctx);ctx.save();ctx.translate(360+studioObjectX,360+studioObjectY);ctx.rotate(studioObjectRotation*Math.PI/180);ctx.scale(studioObjectScale,studioObjectScale);ctx.drawImage(studioWorkCanvas,-360,-360);ctx.restore();if(studioBg==='transparent')itemWorkingPhoto=out.toDataURL('image/png');else{itemWorkingPhoto=out.toDataURL('image/webp',.86);if(!itemWorkingPhoto.startsWith('data:image/webp'))itemWorkingPhoto=out.toDataURL('image/jpeg',.88)}itemCutoutApplied=studioMode!=='original'||maskHasPaint(studioManualEraseMask)||maskHasPaint(studioManualRestoreMask);itemStudioState=studioLegacyMode?null:{version:2,sourceFingerprint:photoFingerprint(itemOriginalPhoto||''),mode:studioMode,edge:studioEdge,eraseMask:maskDataURL(studioManualEraseMask),restoreMask:maskDataURL(studioManualRestoreMask),scale:studioObjectScale,x:studioObjectX,y:studioObjectY,rotation:studioObjectRotation,bg:studioBg,customBg:studioCustomBg};showPhoto('#itemPhotoPreview','#photoPlaceholder',itemWorkingPhoto);updateOriginalPhotoButton();$('#photoStudioDialog').close();$('#scanStatus').textContent=studioBg==='transparent'?'Photo Studio image applied · transparency preserved.':'Photo Studio image applied · standardized square crop.';toast('Photo applied')}
...
function keepMainForeground(mask,w,h){const seen=new Uint8Array(w*h),components=[];const q=new Int32Array(w*h);for(let start=0;start<w*h;start++)if(mask[start]&&!seen[start]){let head=0,tail=0,area=0,border=0;seen[start]=1;q[tail++]=start;const pixels=[];while(head<tail){const p=q[head++],x=p%w,y=(p/w)|0;pixels.push(p);area++;if(x===0||y===0||x===w-1||y===h-1)border=1;const ns=[x>0?p-1:-1,x<w-1?p+1:-1,y>0?p-w:-1,y<h-1?p+w:-1];for(const n of ns)if(n>=0&&mask[n]&&!seen[n]){seen[n]=1;q[tail++]=n}}components.push({area,border,pixels})}if(!components.length)return mask;components.sort((a,b)=>b.area-a.area);const largest=components[0].area||1;const out=new Uint8Array(w*h);for(const comp of components){if(comp.area>=largest*0.12||(!comp.border&&comp.area>=largest*0.04)){for(const p of comp.pixels)out[p]=1}}return out}
...
function buildCutoutPass(im,w,h,edge=45,mode='quick'){const d=im.data;const {bg,mad}=borderBackgroundModel(d,w,h);const passable=new Uint8Array(w*h),strength=new Uint8ClampedArray(w*h);const base=mode==='clean'?0.78:0.65,gradLimit=(mode==='clean'?28:24)+edge*.33,floodThresh=Math.max(16,edge*base+mad*.6),sureFg=floodThresh+(mode==='clean'?22:16);for(let y=0;y<h;y++)for(let x=0;x<w;x++){const p=y*w+x,i=p*4,r=d[i],g=d[i+1],b=d[i+2],dr=r-bg[0],dg=g-bg[1],db=b-bg[2],dist=Math.sqrt(dr*dr+dg*dg+db*db),xr=Math.min(w-1,x+1),yd=Math.min(h-1,y+1),ir=(y*w+xr)*4,id=(yd*w+x)*4,grad=Math.max(Math.abs(r-d[ir])+Math.abs(g-d[ir+1])+Math.abs(b-d[ir+2]),Math.abs(r-d[id])+Math.abs(g-d[id+1])+Math.abs(b-d[id+2]))/3;passable[p]=(dist<floodThresh&&grad<gradLimit)?1:0;strength[p]=clamp(Math.round((dist-floodThresh)/(sureFg-floodThresh)*255),0,255)}const bgFlood=new Uint8Array(w*h),q=new Int32Array(w*h);let head=0,tail=0;const push=p=>{if(p<0||p>=w*h||bgFlood[p]||!passable[p])return;bgFlood[p]=1;q[tail++]=p};for(let x=0;x<w;x++){push(x);push((h-1)*w+x)}for(let y=0;y<h;y++){push(y*w);push(y*w+w-1)}while(head<tail){const p=q[head++],x=p%w,y=(p/w)|0;if(x>0)push(p-1);if(x<w-1)push(p+1);if(y>0)push(p-w);if(y<h-1)push(p+w)}let mask=new Uint8Array(w*h);for(let p=0;p<w*h;p++)mask[p]=bgFlood[p]?0:1;mask=fillMaskHoles(mask,w,h);if(mode==='clean'){mask=keepMainForeground(mask,w,h);mask=closeMask(mask,w,h)}const alpha=new Uint8ClampedArray(w*h);for(let p=0;p<w*h;p++){if(!mask[p]){alpha[p]=0;continue}alpha[p]=isMaskBoundary(mask,w,h,p)?Math.max(mode==='clean'?210:190,strength[p]):255}return {alpha,mask}}
