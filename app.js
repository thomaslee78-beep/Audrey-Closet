...
async function init(){
  ...
  if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=13.12-dev1',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{});
  renderAll();
}
...
function bindDialogs(){
  ...
  ['itemPhoto','itemPhotoLibrary','reviewItemPhoto','reviewItemPhotoLibrary'].forEach(inputId=>{const input=$('#'+inputId);input.addEventListener('click',()=>{itemPhotoPickerActive=true;ensureItemDialogVisible()})});
...
function loadItemIntoEditor(item=null,preferredCategory=''){
  ...
  $('#reviewPhotoMenuWrap').classList.toggle('hidden',!isEdit);$('#smartScanIconBtn').classList.toggle('hidden',!itemWorkingPhoto);closeReviewPhotoMenu();updateReviewPhotoMenuState();
...
async function handleItemPhotoSelection(e,source='camera'){
  ...
  try{
    ...
    showPhoto('#itemPhotoPreview','#photoPlaceholder',itemWorkingPhoto);updateOriginalPhotoButton();$('#smartScanIconBtn').classList.remove('hidden');updateReviewPhotoMenuState();
  ...
  finally{setPhotoBusy(false);e.target.value='';updateReviewPhotoMenuState()}
}
...
function updateReviewPhotoMenuState(){const restore=$('#reviewRestoreOriginalBtn');if(restore)restore.classList.toggle('hidden',!itemOriginalPhoto||itemWorkingPhoto===itemOriginalPhoto);const scan=$('#smartScanIconBtn');if(scan)scan.classList.toggle('hidden',!itemWorkingPhoto)}
