...
function beginPortfolioPress(card,folder,pointerId,x,y,touchId=null){resetPortfolioDrag();const aggregate=folder==='All'||folder==='Favorites';portfolioDrag={...portfolioDrag,card,folder:aggregate?'':folder,pointerId,touchId,startX:x,startY:y,x,y,aggregate,aggregateLongPress:false};portfolioDrag.timer=setTimeout(()=>{if(portfolioDrag.aggregate){portfolioDrag.timer=null;portfolioDrag.aggregateLongPress=true;suppressPortfolioClickUntil=Date.now()+1600;navigator.vibrate?.(12);toast('Choose a specific portfolio category before reordering looks.');return}startPortfolioDrag()},430)}
