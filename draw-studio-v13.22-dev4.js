/* Audrey Closet v13.22 Draw Studio dev4
 * First live drawing build: Pencil freeform only.
 * Other tools remain UI-only placeholders.
 * Does not replace drawBoard(); wraps item rendering/normalization only.
 */
(function(){
  'use strict';

  const ROOT_ID='drawStudioDev4';
  const STYLE_ID='drawStudioDev4Styles';
  const TOOLS=[
    ['line','Line','╱'],
    ['arrow','Arrow','➜'],
    ['pencil','Pencil','✎'],
  ...
  const state={tool:'pencil',thickness:2,color:'#4f514a',lineStyle:'solid',arrowEnds:'single',drawMode:false};
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      #outfitBoard.draw-dev4-active{touch-action:none!important;cursor:crosshair}
      #outfitBoard .draw-live-overlay-dev4{position:absolute;inset:0;width:100%;height:100%;z-index:2147483000;pointer-events:none;overflow:visible}
      .board-piece .drawing-svg-dev4{display:block;width:100%;height:100%;overflow:visible;pointer-events:none}
  ...
  function installDrawingRenderer(){
    ...
    boardItemContent=function(b){
      if(b&&b.kind==='drawing'&&b.drawingType==='freeform'){
        ...
        return `<svg class="drawing-svg-dev4" viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" aria-hidden="true"><polyline points="${esc(b.points||'')}" fill="none" stroke="${color}" stroke-width="${width}" stroke-linecap="round" stroke-linejoin="round"${dash?` stroke-dasharray="${dash}"`:''} opacity="${opacity}" vector-effect="non-scaling-stroke"/></svg>`;
      }
  ...
  function pointOnBoard(board,e){
    ...
    return {
      x:Math.max(0,Math.min(board.clientWidth,(e.clientX-rect.left)*sx)),
      y:Math.max(0,Math.min(board.clientHeight,(e.clientY-rect.top)*sy))
    };
  }

  function ensureOverlay(board){
    let svg=board.querySelector('.draw-live-overlay-dev4');
    if(svg)svg.remove();
    svg=document.createElementNS('http://www.w3.org/2000/svg','svg');
    svg.setAttribute('class','draw-live-overlay-dev4');
  ...
  function finishStroke(board,e,cancelled){
    ...
    if(stroke.points.length===1){
      ...
      stroke.points.push({x:p.x+.01,y:p.y+.01});
    }
    ...
    const maxX=Math.min(board.clientWidth,Math.max(...xs)+pad);
    const maxY=Math.min(board.clientHeight,Math.max(...ys)+pad);
    ...
    const item={
      uid:id(),kind:'drawing',drawingType:'freeform',tool:'pencil',
      strokeColor:state.color,strokeWidth:state.thickness,strokeStyle:state.lineStyle,opacity:1,
      points:relative,x:minX,y:minY,w,h,rotation:0,z:nextZ()
    };
  ...
  function applyDrawMode(){
    const board=document.getElementById('outfitBoard');
    if(board)board.classList.toggle('draw-dev4-active',state.drawMode&&state.tool==='pencil');
  ...
  function sync(root){
    ...
    const slider=root.querySelector('#drawThicknessDev4');
    const value=root.querySelector('.draw-thickness-value');
    const color=root.querySelector('#drawColorDev4');
    ...
    if(modeBtn){
      modeBtn.disabled=!pencilReady;
      modeBtn.classList.toggle('active',state.drawMode&&pencilReady);
      modeBtn.textContent=state.drawMode&&pencilReady?'Done':'Draw';
      modeBtn.setAttribute('aria-pressed',state.drawMode&&pencilReady?'true':'false');
    }
    ...
    if(status){
      ...
      status.textContent=`${match?match[1]:state.tool} · ${state.thickness}px${stylePart}`;
    }
    if(hint){
      if(state.tool==='pencil')hint.textContent=state.drawMode?'Draw continuously on the board · tap Done to select objects':'Pencil is live in dev4 · tap Draw to begin';
      else hint.textContent='This tool is UI-only in dev4';
    }
    applyDrawMode();
  }
  ...
  function buildRoot(){
    ...
    root.id=ROOT_ID;
    root.innerHTML=`
      <div class="draw-clean-head"><span>Draw Studio</span><small>dev4 · Pencil live</small></div>
      ...
      <div class="draw-control-row">
        <label class="draw-thickness"><span class="draw-control-label">Size</span><input id="drawThicknessDev4" type="range" min="1" max="24" step="1" value="2" aria-label="Draw thickness"><span class="draw-thickness-value">2</span></label>
        <label class="draw-color-wrap" aria-label="Draw color"><input id="drawColorDev4" type="color" value="#4f514a"></label>
      ...
      <div class="draw-secondary-row">
        ...
        <button type="button" class="draw-mode-btn" aria-pressed="false">Draw</button>
      </div>`;
    ...
    root.querySelectorAll('.draw-tool-btn').forEach(btn=>{
      btn.addEventListener('click',()=>{
        ...
        if(state.tool!=='pencil')state.drawMode=false;
        sync(root);
      });
    });
    root.querySelector('#drawThicknessDev4')?.addEventListener('input',e=>{
      state.thickness=Math.max(1,Math.min(24,Number(e.target.value)||1));
      sync(root);
    });
    root.querySelector('#drawColorDev4')?.addEventListener('input',e=>{
      state.color=e.target.value||'#4f514a';
      sync(root);
    });
    root.querySelectorAll('[data-line-style]').forEach(btn=>{
      btn.addEventListener('click',()=>{state.lineStyle=btn.dataset.lineStyle||'solid';sync(root);});
    });
    root.querySelectorAll('[data-arrow-ends]').forEach(btn=>{
      btn.addEventListener('click',()=>{state.arrowEnds=btn.dataset.arrowEnds||'single';sync(root);});
    });
    root.querySelector('.draw-mode-btn')?.addEventListener('click',()=>{
  ...
  function scheduleInstall(){requestAnimationFrame(()=>requestAnimationFrame(install));}
