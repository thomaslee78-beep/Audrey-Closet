...
(function(){
  ...
  const ROOT_ID='drawStudioCleanDev2';
  ...
  function installDrawStudio(){
    ...
    if(root && !content.contains(root))root.remove();
    if(!root){
      root=buildRoot();
  ...
  function start(){
    installDrawStudio();
    const observer=new MutationObserver(()=>installDrawStudio());
    observer.observe(document.documentElement,{childList:true,subtree:true});
    document.addEventListener('click',e=>{
      if(e.target.closest?.('.decorate-studio-tab[data-decorate-group="draw"]')){
        requestAnimationFrame(installDrawStudio);
      }
    },true);
  }
